...
/**
 * Colección local de GIFs con datos de ejemplo
 */
export const gifCollection: Gif[] = [
  {
    id: 'gif-001',
...
/**
 * Obtiene todos los GIFs de la colección
 */
export function getAllGifs(): Gif[] {
  return [...gifCollection];
}

/**
 * Busca GIFs por término de búsqueda
 * Busca en título, autor, etiquetas y descripción (ignorando mayúsculas y espacios)
 */
export function searchGifs(searchTerm: string): Gif[] {
  ...
  return gifCollection.filter((gif) => {
    // Buscar en título
    const titleMatch = gif.title.toLowerCase().includes(normalizedSearch);

    // Buscar en autor (si existe)
    ...
    const descriptionMatch = gif.description
      ? gif.description.toLowerCase().includes(normalizedSearch)
      : false;
...
/**
 * Obtiene un GIF por su ID
 */
export function getGifById(id: string): Gif | undefined {
  return gifCollection.find((gif) => gif.id === id);
}
